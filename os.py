...
play_src = dict(
            name="Show openstack details",
            hosts="mitaka",
            gather_facts="no",
            become="true",
            tasks=[
                #dict(name="Gather facts about all ports", action=dict(module="os_project_facts", auth=dict(auth_url="http://192.168.20.208:5000/v2.0",
                #username="admin", password="cnetopenstack", project_name="admin"))),

                #dict(name="Show openstack projects", action=dict(module="debug", var="openstack_projects")),
